...
class mySensor{
	private:
		LCD* display_handle; 
		uint16_t temp_inC;				//BASE wird gespeichert, fuer alarm vergleichung == temp;
		uint16_t temp_inF;				// nur fuer Anzeige
		uint16_t humid;
		uint16_t alarm_value_inC;		//BASE wird gespeichert == alarm_value
...
inline void led_blinken(){				// BUG: Nachdem Benutzung des Drehgebers, ist die Funktion nicht aufgerufen.
	if (1 > led_delay){
		if(do_blink){
